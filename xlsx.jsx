...
function ExcelIntegration({user}){
    ...
    function convertJSON({Name , tillTotal, pendingBills,uber,cash ,cardBarclays,cardAmex, Maruthi,shop,staff,pending}){//using props
        const date = new Date()//getting date and time
        let total = parseInt(tillTotal) + parseInt(pendingBills)
        let grandtotal = parseInt(uber) + total
        let difference = total - (parseInt(cash) + parseInt(cardBarclays) + parseInt(cardAmex) + parseInt(Maruthi) + parseInt(shop) + parseInt(pending) + parseInt(staff))
        const JSONobj = {
          Date: date,
          Name: Name,
    ...
    function Submit(){
        axios.post('https://sheet.best/api/sheets/c6fe3a18-2297-46c9-9cc7-ffca0c755109', Entries)
        .then(response => {
          ...
          <div className="entireForm">
              ...
              <div className="everyForm">
               ...
               <label htmlFor="Maruthi ">Maruthi:</label>
                ...
                <button className="button-10" onClick={() => convertJSON({Name, tillTotal, pendingBills,uber, cash, cardBarclays,cardAmex, Maruthi,shop,pending,staff})} >Submit</button>
                { submit ? <div className = "buttonWrapper"><button className ="confirmButton button-10" onClick={() => Submit(Entries)}>Confirm Submit </button></div> : <p className="smallFont">SugarTech 2023©</p>  }
              </div>
             </div>
